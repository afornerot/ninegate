...
# 4. Generate all INSERT statements (with ON CONFLICT DO NOTHING)
echo "[4/6] Generating INSERTs..."
...
$MYSQL "SELECT CONCAT('INSERT INTO item (id, title, summary, bgcolor, color, url, new_tab, sort_order, icon_id, category_id) VALUES (', id, ', ', QUOTE(title), ', ', COALESCE(QUOTE(subtitle), 'NULL'), ', ', COALESCE(QUOTE(color), 'NULL'), ', ', COALESCE(QUOTE(color), 'NULL'), ', ', QUOTE(url), ', ', IF(target = '_blank', 'true', 'false'), ', ', COALESCE(rowOrder, 0), ', NULL, ', category, ') ON CONFLICT (id) DO NOTHING;') FROM item ORDER BY id;" > /tmp/migrate_items.sql 2>/dev/null
...
echo "[5/6] Assembling SQL..."
cat /tmp/migrate_groups.sql /tmp/migrate_users.sql /tmp/migrate_usergroups.sql /tmp/migrate_itemcategories.sql /tmp/migrate_items.sql /tmp/migrate_itemgroups.sql /tmp/migrate_blogs.sql /tmp/migrate_bookmarks.sql > "$FINAL_SQL"
echo "UPDATE app_group SET slug = 'all', is_system = true WHERE name = 'Tout le Monde';" >> "$FINAL_SQL"

# Fix MySQL escaping: backslash-quote → doubled single-quote (for PostgreSQL)
sed -i "s/\\\\\\\\'/''/g" "$FINAL_SQL"

# 6. Apply
echo "[6/6] Applying to PostgreSQL..."
