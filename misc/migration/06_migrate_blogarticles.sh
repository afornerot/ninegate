#!/bin/bash
# Step 6: Migrate blog articles (blog "Interne" only)
...
echo "=== [6/9] Migrate blog articles (Interne) ==="
...
docker exec migration-mariadb mysql -uroot -proot ninegate -N -e "SELECT CONCAT(id, '|', COALESCE(name, ''), '|', COALESCE(user_id, 'NULL'), '|', submit) FROM blogarticle WHERE blog_id = 3 ORDER BY id;" > /tmp/articles_meta.txt 2>/dev/null

# Step 2: Export HTML content
docker exec migration-mariadb mysql -uroot -proot ninegate --raw -e "SELECT id, COALESCE(description, '') FROM blogarticle WHERE blog_id = 3 ORDER BY id;" > /tmp/articles_html_raw.txt 2>/dev/null
...
while IFS='|' read -r id name user_id submit; do
    ...
    echo "INSERT INTO blog_article (id, title, slug, content, image, \"created_at\", \"updated_at\", user_id, blog_id) VALUES ($id, '$NAME_ESC', '$SLUG', '$MD', NULL, '$submit', '$submit', $user_id, 3) ON CONFLICT (id) DO NOTHING;" >> "$FINAL"
done < /tmp/articles_meta.txt
...
echo "✓ $ACOUNT blog articles migrated (blog Interne)"
